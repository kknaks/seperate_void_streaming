...
## §3 시나리오 — 파일 재생 master clock end-to-end
...
sequenceDiagram
    ...
    UI->>WS: audio.ended → text {"type":"eof"}
    UI->>WS: done 수신 후 WS close
...
| `demo-ui` | `demo-ui` | `web/` | 파일 업로드 + `<audio>` 재생 master clock + AudioWorklet capture (Float32→Int16) + WS 연결 + 라이브 발화 로그 표시 (spec-07 §4·§5) |
